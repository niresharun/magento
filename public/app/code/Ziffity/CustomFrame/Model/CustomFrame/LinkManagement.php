...
use Magento\Bundle\Model\LinkManagement as CoreLinkManagement;

/**
 * Class used to manage customframe products links.
 *
 * @SuppressWarnings(PHPMD.CouplingBetweenObjects)
 */
class LinkManagement extends CoreLinkManagement implements ProductLinkManagementInterface
{
    /**
     ...
     */
    public function saveChild($sku, LinkInterface $linkedProduct)
    {
        $product = $this->productRepository->get($sku, true);
        if ($product->getTypeId() != Type::TYPE_CODE) {
            throw new InputException(
                __('The product with the "%1" SKU isn\'t a customframe product.', [$product->getSku()])
            );
        }
        ...
        if ($linkProductModel->isComposite()) {
            throw new InputException(__('The customframe product can\'t contain another composite product.'));
        }
     ...
     * @param string|int $linkedProductId
     * @param string|int $parentProductId
     *
     * @return Selection
     *
     * @SuppressWarnings(PHPMD.CyclomaticComplexity)
    ...
    ): Selection
    {
        ...
        if ($productLink->getProductQuantityClassification() !== null) {
            $selectionModel->setProductQuantityClassification($productLink->getProductQuantityClassification());
        }
        if ($productLink->getProductQuantityCalculation() !== null) {
            $selectionModel->setProductQuantityCalculation($productLink->getProductQuantityCalculation());
        }
    ...
    public function removeChild($sku, $optionId, $childSku)
    {
        ...
        if ($product->getTypeId() != Type::TYPE_CODE) {
            throw new InputException(__('The product with the "%1" SKU isn\'t a customframe product.', $sku));
        }
        ...
        if (empty($removeSelectionIds)) {
            throw new NoSuchEntityException(
                __("The customframe product doesn't exist. Review your request and try again.")
            );
        }
        $linkField = $this->metadataPool->getMetadata(ProductInterface::class)->getLinkField();
        /* @var $resource Bundle */
