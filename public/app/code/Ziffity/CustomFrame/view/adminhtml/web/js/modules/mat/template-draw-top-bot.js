...
var DRAW_TOP_BOT = (function () {
    ...
    TopBot.prototype.setModEls = function () {
        var tab_wrap = this.cs.closest('[id*="product_info_tabs_"]'),
            mod_name = this.name.indexOf('header') === -1 ? this.name : 'header';

        this.els.tool_wr  = tab_wrap.querySelector('.admin-topbot-wr');
        this.els.width    = tab_wrap.querySelector('[name*="[size][width]"]');
        this.els.height   = tab_wrap.querySelector('[name*="[size][height]"]');
        this.els.position = tab_wrap.querySelector('[name*="[position]"]');
        this.els.font_size_def  = tab_wrap.querySelector('[name*="[font_size][default]"]');
        this.els.font_size_min  = tab_wrap.querySelector('[name*="[font_size][min]"]');
        this.els.font_size_step = tab_wrap.querySelector('[name*="[font_size][step]"]');
        this.els.tab_link       = document.getElementById('product_info_tabs_' + mod_name);
        this.els.prod_t         = document.querySelector('select[name*="product[size_type]"]');
        this.els.prod_w         = document.querySelector('select[name*="product[width]"]');
        this.els.prod_h         = document.querySelector('select[name*="product[height]"]');
        this.els.mat_top_int    = document.querySelector('select[name*="openings-mat-size-top-integer"]');
        this.els.mat_top_tenth  = document.querySelector('select[name*="openings-mat-size-top-tenth"]');
        this.els.mat_rev        = document.querySelector('select[name*="product[openings-mat-size-reveal]"]');
        this.els.mat_over       = document.querySelector('select[name*="product[matboard_overlap]"]');
        this.els.op_type        = document.querySelectorAll('input[name*="opadmin-type"]');
    };
    ...
    TopBot.prototype.setElsEvents = function () {
        ...
        this.els.tab_link.addEventListener('mousedown', function () {
            was_active = this.className.indexOf('active') === -1;
        });
        this.els.tab_link.addEventListener('click', function () {
            if (was_active) {
                updateCoreVars();
                this.update();
                was_active = false;
        ...
        if (typeof OPENINGS_ADMIN !== 'undefined') {
            OPENINGS_ADMIN.addListener('changed_mat_size', function () {
                updateCoreVars();
                this.update();
            }.bind(this));
            OPENINGS_ADMIN.addListener('changed_op_type', function () {
                updateCoreVars();
                this.update();
            }.bind(this));
        } else {
    ...
    TopBot.prototype.renderOpeningsMult = function (op_list) {
        var list  = (typeof OPENINGS_ADMIN === 'undefined') ? op_list : OPENINGS_ADMIN.exportList(),
            ratio = this.size_px.ratio;
    ...
    TopBot.prototype.updateVars = (function () {
        ...
        var updateVarsInches = function (m_height) {
            ...
            if (has_width) {
                if (op_type !== 'multiple') {
                    this.els.width.closest('tr').style.display = 'none';
                } else {
                    this.els.width.closest('tr').style.display = 'table-row';
                }
            }
