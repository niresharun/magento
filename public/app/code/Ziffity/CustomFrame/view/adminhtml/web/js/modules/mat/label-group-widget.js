define([
    ...
    'jquery/ui',
    "webfont",
], function(registry,uiAlert,$t,$,Fraction, Vue, VueResource,_) {
    "use strict";
    Vue.use(VueResource);
    ...
    var DRAW_TOP_BOT = (function () {
        ...
        TopBot.prototype.setModEls = function () {
            var tab_wrap = document.querySelector('[data-index="label_group"]'),
                mod_name = this.name.indexOf('header') === -1 ? this.name : 'header';

            this.els.tool_wr  = tab_wrap.querySelector('.admin-topbot-wr');
            this.els.width    = document.querySelector('[name*="label_width"]');//label_width
            this.els.height   = document.querySelector('[name*="label_height"]');//label_height
            this.els.position = document.querySelector('[name*="label_position"]');//label_position
        ...
        TopBot.prototype.setElsEvents = function () {
            ...
            $('button[data-index="label_modal_button"]').on('click',function(){
                updateCoreVars();
                self.update();
            });
        ...
        TopBot.prototype.updateVars = (function () {
            ...
            var updateVarsInches = function (m_height) {
                ...
                if (has_width) {
                    if (op_type !== 'multiple') {
                        registry.get('index=label_width').visible(false);
                    } else {
                        registry.get('index=label_width').visible(true);
                    }
                }
    ...
    $.widget('custom.labelVue', {
        _create: function() {
            this.setLabelUiData();
            DRAW_TOP_BOT.init();
            window.labelObjectData = DRAW_TOP_BOT;
        },
        setLabelUiData:function(){
            let UiData = window.productJson.modules.labels.UiData;
            registry.get('index=label_position').value(UiData.position);
            registry.get('index=label_height').value(UiData.height);
            if (UiData.width!==undefined) {
                registry.get('index=label_width').value(UiData.width);
            }
            registry.get('index=label_font_size_min').value(UiData.minimal_font_size);
            registry.get('index=label_font_size_step').value(UiData.font_size_step);
            registry.get('index=label_font_size_default').value(UiData.default_font_size);
            return;
        }
    });
    return $.custom.labelVue;
});
