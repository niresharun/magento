...
use Magento\Checkout\Model\DefaultConfigProvider as CoreConfigProvider;
use Ziffity\ProductCustomizer\Helper\Selections;
use Ziffity\SavedDesigns\ViewModel\ProcessSaveDesigns;
use Magento\Catalog\Api\ProductRepositoryInterface;
use Ziffity\ProductCustomizer\Model\Checkout\ImageProvider;

/**
 * Default Config Provider for checkout
 */
class DefaultConfigProvider extends CoreConfigProvider
{
    /**
    ...
    private function getQuoteItemData()
    {
        ...
        if ($quoteId) {
            ...
            foreach ($quoteItems as $index => $quoteItem) {
                ...
                if($quoteItem->getProductType() === "customframe"){
                    if($quoteItem->getAdditionalData()){
                        $selectionsHelper = $this->selectionsHelper;
                        $steps = $selectionsHelper->getCompletedStepsFromOptions($quoteItem->getAdditionalData());
                        $addData =  $selectionsHelper->getUnserializedData($quoteItem->getAdditionalData());
                        $product = $this->productRepository->getById($quoteItem->getProductId());
                        $quoteItemData[$index]['selections_data'] = $selectionsHelper->getSelections($addData, $product, $steps );
                        if(
                            !empty($addData['additional_data']['canvasData']) &&
                            $this->savedImg->checkImageExists($addData['additional_data']['canvasData'])
                        ) {
                            $quoteItemData[$index]['saved_img'] = $this->savedImg->getImagePath($addData['additional_data']['canvasData']);
                        }
                    }
                }
