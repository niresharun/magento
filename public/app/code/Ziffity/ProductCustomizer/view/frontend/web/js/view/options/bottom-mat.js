define([
    ...
    return AbstractOption.extend({
        defaults: {
            ...
            bottomMatProducts: ko.observableArray([]),
            productSelection:ko.observable(),
            placeholder:ko.observable('Search for product name...'),
            searchFocused:ko.observable(false),
            defaultSelection: window.customizerConfig.options.mat.active_items.bottom_mat,
            optionId: window.customizerConfig.options.mat.active_items.bottom_mat.option_id,
            ...
            exports: {
                optionId: '${ $.provider }:options.mat.bottom_mat.option_id',
                productSelection: '${ $.provider }:options.mat.active_items.bottom_mat',
            ...
            imports: {
                bottomMatProducts: '${ $.provider }:options.mat.bottomMat.products',
        ...
        initSelection: function() {
            var self = this;
            self.productSelection(customizerDataResolver.resolveBottomMatSelection());
        },
        ...
        initialize: function() {
            ...
            ko.computed(function() {
                self.bottomMatPrice(self.productSelection().price);
            });
            ...
            this.isActive.subscribe(function(value){
                if (value) {
                    self.recalculatePrice(self);
                    //if (!self.bottomMatProducts().length) {
        ...
        loadProductListIntoObservable:function() {
            ...
            data.option = 'mat';
            data.optiontype = 'bottom-mat';
            data.active_item = self.options.mat.active_items.bottom_mat;
            ...
            result.done(function(response){
                if (response!==undefined && response.products !== undefined && response.active_item) {
                    let tempProduct = {};
                    response.products['bottom_mat'].forEach(product =>{
                    ...
                    self.bottomMatProducts(response.products['bottom_mat']);
                    if (response.product_total_count!==undefined) {
                        self.productListCount(response.product_total_count.bottom_mat);
