define([
    ...
    return AbstractOption.extend({
        defaults: {
            ...
            laminateInteriorProducts:ko.observableArray([]),
            ...
            exports: {
                productSelection: '${ $.provider }:options.laminate_finish.active_items.laminate_interior',
                optionId: '${ $.provider }:options.laminate_finish.laminate_interior.option_id',
            },
            imports: {
        ...
        initSelection: function() {
            var self = this;
            self.productSelection(customizerDataResolver.resolveInteriorLaminateSelection());
        },
        ...
        initLaminateInteriorTab: function () {
            let self = this;
            let code = 'Interior Laminate';
            laminateTypes.registerLaminate(code, self.isActive, self.sortOrder,
                self.selection, self.defaultSelection,
                require.toUrl('Ziffity_ProductCustomizer/images/laminate/laminate-interior.png'));
        },
        ...
        loadProductListIntoObservable:function() {
            ...
            data.optiontype = 'laminate-interior';
            ...
            result.done(function(response){
                if (response!==undefined && response.products !== undefined) {
                    response.products.laminate_interior.forEach(product =>{
                        if(product.id == self.productSelection().id) {
                            self.productSelection.silentUpdate(product);
                        }
                    });
                    self.laminateInteriorProducts(response.products.laminate_interior);
                    // _.each(response.products.laminate_interior,function(item){
                    ...
                    if (response.product_total_count!==undefined) {
                        self.productListCount(response.product_total_count.laminate_interior);
